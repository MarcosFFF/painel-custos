...
def _risco_valor_por_paciente_temp(pasta="."):
    ...
    (CD_PRESTADOR), MÊS A MÊS (não colapsado ainda) — usado só pelo Indicador 7
    (Dependência de Pacientes) da aba "Índice de Risco". O `agregado` que
    carregar_base_severidade() devolve nunca tem essa combinação (é agrupado por
    MES/UF/ESPECIALIDADE/... — perde o paciente individual) e `base_usuarios` tem
    CD_USUARIO mas não VL_PAGO. Em vez de mexer em severidade.py pra expor isso,
    lemos os mesmos CSVs de novo aqui, só com as colunas necessárias, reaproveitando
    o parser (_ler_csv_parte/COLUNAS_ESPERADAS) que já é importado de lá — mesmo
    padrão já usado por _carregar_crosswalk_cidade_cluster_temp logo acima (relê a
    fonte, não toca no módulo). Cacheado porque é leitura de disco — a leitura bruta
    (a parte cara) não depende do filtro de Mês da página; quem chama filtra por MES
    e re-soma DEPOIS, em memória (barato), reagindo ao filtro sem reler CSV nenhum.
    MES calculado a partir de DATA_SOL, do mesmo jeito que severidade.py calcula pro
    `agregado` (dados["MES"] = DATA_REF.dt.strftime("%Y-%m"), onde DATA_REF acaba
    sendo sempre DATA_SOL ali) — assim os valores de MES batem entre as duas bases.
    """
    candidatos = sorted(
        glob.glob(os.path.join(pasta, "*4016R.csv"))
        + glob.glob(os.path.join(pasta, "*4016R_parte_*.csv"))
    )
    colunas_vazias = ["CD_PRESTADOR", "CD_USUARIO", "MES", "VL_PAGO"]
    if not candidatos:
    ...
    for arq in candidatos:
        try:
            ...
            continue
        parte = parte.copy()
        parte["MES"] = parte["DATA_SOL"].dt.strftime("%Y-%m")
    ...
    return (
        bruto.groupby(["CD_PRESTADOR", "CD_USUARIO", "MES"], dropna=False, observed=True)["VL_PAGO"]
        .sum().reset_index()
    )
...
def _calcular_indicadores_risco_temp(agregado_universo, usuarios_universo, valor_paciente_temp):
    ...
    # de grau 1 (conferido em verify_risco.py, caso "CRESCIMENTO_FORTE"). Com o filtro
    # de Mês da página agora valendo pra esta aba (agregado_universo já vem filtrado),
    # um período de 1 mês só naturalmente não tem os 3 meses mínimos pro I4 (vira NaN,
    # não penaliza nem quebra) — pra ver tendência com um único mês selecionado, use o
    # histórico mês a mês do prestador (abaixo, na aba, quando 1 prestador é filtrado).
    ...
    # Vetorizado (sem .apply() por prestador). `valor_paciente_temp` já chega aqui
    # filtrado pelo Mês da página quando aplicável (ver "with tab_risco_temp:" — o
    # filtro é aplicado em cima do resultado cacheado de _risco_valor_por_paciente_temp,
    # sem reler CSV nenhum).
...
# ============================================================
    ...
    with tab_risco_temp:
        ...
        st.caption(
            "Respeita o filtro de Mês da página (1 mês ou vários) — assim como Plano/"
            "Especialidade, se aplicados. Com só 1 mês selecionado, o I4 (tendência) "
            "fica sem histórico suficiente e entra como \"—\" (não penaliza nem "
            "favorece) — pra ver a evolução mês a mês do prestador mesmo assim, "
            "filtre por 1 prestador específico abaixo: aparece um histórico completo "
            "(todos os meses carregados, independente do filtro de Mês).\n\n"
            "Prestadores com valor pago abaixo do piso de materialidade (mediana do "
        ...
        if "risco_resultado_temp" not in st.session_state:
            ...
            if _risco_tabela_completa_temp.empty:
                ...
                # _risco_filtrado_geo_temp = só os filtros de UF/Cidade/Cluster/Prestador
                # acima (sem o checkbox "mostrar Sem alerta") — é a base do resumo/gráfico
                # por faixa logo abaixo, pra sempre mostrar a distribuição completa das 5
                # faixas reagindo ao que foi filtrado (ex.: "filtrei a cidade tal — quantos
                # ficaram verde, quantos moderado alto...").  _risco_exibicao_temp (tabela +
                # detalhamento, mais abaixo) aplica o checkbox por cima disso.
                _risco_filtrado_geo_temp = _risco_tabela_completa_temp.copy()
                if f_uf_risco_temp:
                    _risco_filtrado_geo_temp = _risco_filtrado_geo_temp[
                        _risco_filtrado_geo_temp["UF"].isin(f_uf_risco_temp)
                    ]
                if f_cidade_risco_temp:
                    _risco_filtrado_geo_temp = _risco_filtrado_geo_temp[
                        _risco_filtrado_geo_temp["CIDADE_PRESTADOR"].isin(f_cidade_risco_temp)
                    ]
                if f_cluster_risco_temp:
                    _risco_filtrado_geo_temp = _risco_filtrado_geo_temp[
                        _risco_filtrado_geo_temp["CLUSTER"].isin(f_cluster_risco_temp)
                    ]
                if f_prestador_risco_temp:
                    _risco_filtrado_geo_temp = _risco_filtrado_geo_temp[
                        _risco_filtrado_geo_temp["NOME_PRESTADOR"].isin(f_prestador_risco_temp)
                    ]

                _risco_exibicao_temp = _risco_filtrado_geo_temp
                ...
                # ---------- resumo + gráfico por faixa (reage aos filtros de UF/Cidade/
                # Cluster/Prestador acima) ----------
                st.markdown(
                    f"**Resumo — prestadores por faixa** (com os filtros acima: "
                    f"{len(_risco_filtrado_geo_temp)} prestador(es))"
                )
                _contagem_faixa_temp = _risco_filtrado_geo_temp["classificacao"].value_counts()
                ...
                _df_grafico_faixa_temp = pd.DataFrame([
                    {"Faixa": _rotulo_gf_temp, "Prestadores": int(_contagem_faixa_temp.get(_rotulo_gf_temp, 0)),
                     "Cor": _cor_gf_temp}
                    # invertido (Sem alerta -> Alerta forte) pra ficar crescente da esquerda
                    # pra direita no gráfico, do menos grave pro mais grave.
                    for _, _rotulo_gf_temp, _cor_gf_temp, _ in reversed(_RISCO_FAIXAS_TEMP)
                ])
                fig_faixa_risco_temp = px.bar(
                    _df_grafico_faixa_temp, x="Faixa", y="Prestadores", text="Prestadores",
                    color="Faixa",
                    color_discrete_map=dict(zip(_df_grafico_faixa_temp["Faixa"], _df_grafico_faixa_temp["Cor"])),
                )
                fig_faixa_risco_temp.update_traces(textposition="outside")
                fig_faixa_risco_temp.update_layout(
                    height=300, margin=dict(l=10, r=10, t=20, b=10), showlegend=False,
                    xaxis_title=None, yaxis_title="Qtde de prestadores",
                )
                st.plotly_chart(
                    fig_faixa_risco_temp, use_container_width=True, key="grafico_faixa_risco_temp",
                )

                # ---------- histórico mês a mês do prestador (só quando 1 prestador é
                # filtrado acima) — usa TODOS os meses carregados na base, ignorando o
                # filtro de Mês da página, especificamente pra dar uma visão de tendência
                # mesmo quando o filtro de Mês está restrito a 1 mês só (onde o I4 sozinho
                # não tem histórico suficiente pra calcular nada). ----------
                if len(f_prestador_risco_temp) == 1:
                    _nome_prestador_hist_risco_temp = f_prestador_risco_temp[0]
                    st.divider()
                    st.markdown(f"**Histórico mês a mês — {_nome_prestador_hist_risco_temp}**")
                    st.caption(
                        "Todos os meses carregados na base (independente do filtro de Mês da "
                        "página) — só pra visualizar a tendência do prestador por fora do I4."
                    )
                    _match_hist_risco_temp = _risco_tabela_completa_temp[
                        _risco_tabela_completa_temp["NOME_PRESTADOR"] == _nome_prestador_hist_risco_temp
                    ]
                    if _match_hist_risco_temp.empty:
                        st.info("Prestador não encontrado no universo calculado.")
                    else:
                        _cd_prestador_hist_risco_temp = _match_hist_risco_temp.iloc[0]["CD_PRESTADOR"]
                        _hist_universo_risco_temp = aplicar_filtros(
                            agregado, planos=f_plano or None, especialidades=f_especialidade or None,
                        )
                        _hist_prestador_risco_temp = _hist_universo_risco_temp[
                            _hist_universo_risco_temp["CD_PRESTADOR"] == _cd_prestador_hist_risco_temp
                        ]
                        _hist_mensal_risco_temp = (
                            _hist_prestador_risco_temp.dropna(subset=["MES"])
                            .groupby("MES", observed=True)["qtd_procedimentos"].sum().reset_index()
                            .sort_values("MES")
                        )
                        if _hist_mensal_risco_temp.empty:
                            st.info("Sem histórico mensal suficiente para esse prestador.")
                        else:
                            _hist_mensal_risco_temp["mes_rotulo"] = _hist_mensal_risco_temp["MES"].map(label_mes)
                            fig_hist_risco_temp = px.line(
                                _hist_mensal_risco_temp, x="mes_rotulo", y="qtd_procedimentos", markers=True,
                                title="Qtd de procedimentos por mês (histórico completo)",
                            )
                            fig_hist_risco_temp.update_layout(
                                height=300, margin=dict(l=10, r=10, t=40, b=10),
                                xaxis_title="Mês", yaxis_title="Qtd de procedimentos",
                            )
                            st.plotly_chart(
                                fig_hist_risco_temp, use_container_width=True, key="grafico_hist_risco_temp",
                            )
                ...
                def _gerar_pdf_risco_temp():
                    ...
                    if f_mes:
                        _filtros_ativos_risco_temp.append(f"Mês: {', '.join(f_mes)}")
                    if f_plano:
                    ...
                    _linhas_meta_risco_temp = [
                        ("Escopo", f"Mês: {', '.join(f_mes)}" if f_mes else "Todos os meses carregados na base"),
