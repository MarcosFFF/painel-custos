...
def calcular_media_nacional(agregado, coluna_dimensao):
    """Média de uso e custo nacional (sem filtros) por dimensão."""
    ...
    r["custo_medio_nacional"] = (r["valor_pago"] / r["qtd_procedimentos"]).round(2)
    return r

def montar_watchlist(df, top_n=20):
    """
    Watchlist com UF, Cidade e Cluster de cada prestador.
    Pontuação 0-100 combinando custo médio, uso médio e tendência de crescimento.
    """
    ...
    # Pontuação composta (0-100) — percentis
    for c in ["custo_medio", "media_uso", "qtd_procedimentos"]:
        por_prestador[f"pct_{c}"] = por_prestador[c].rank(pct=True).fillna(0)
    por_prestador["pontuacao"] = (
        (por_prestador["pct_custo_medio"] * 0.4
         + por_prestador["pct_media_uso"] * 0.3
         + por_prestador["pct_qtd_procedimentos"] * 0.3) * 100
    ).round(0)
    ...
    cols = ["CD_PRESTADOR", "UF", "CIDADE", "CLUSTER", "ESPECIALIDADE",
            "qtd_procedimentos", "valor_pago", "custo_medio", "quantidade_uso",
            "media_uso", "tendencia_pct", "pontuacao"]
...
def identificar_ofensores(df, percentil=0.95):
    """
    Identifica ofensores com justificativa textual.
    ...
    for _, row in por_prestador.iterrows():
        motivos = []
        if row["alerta_custo"]:
            motivos.append(f"custo médio R$ {row['custo_medio']:.2f} no {pct_str} (limiar R$ {limiar_custo:.2f})")
        if row["alerta_volume"]:
            motivos.append(f"volume {int(row['qtd_procedimentos'])} procedimentos no {pct_str} (limiar {int(limiar_volume)})")
        if row["alerta_uso"]:
            motivos.append(f"uso médio {row['media_uso']:.2f} no {pct_str} (limiar {limiar_uso:.2f})")
    ...
    cols = ["CD_PRESTADOR", "UF", "CIDADE", "CLUSTER", "ESPECIALIDADE",
            "qtd_procedimentos", "valor_pago", "custo_medio", "quantidade_uso",
            "media_uso", "alerta_custo", "alerta_volume", "alerta_uso",
            "criterios_atingidos", "relevante", "justificativa"]
    cols = [c for c in cols if c in resultado.columns]
    return resultado[cols].reset_index(drop=True)

def calcular_desvios(df):
    """Desvios de cada prestador vs média da própria especialidade."""
    info = _info_prestador(df)
    ...
    por_prestador["custo_medio"] = (por_prestador["valor_pago"] / por_prestador["qtd_procedimentos"]).round(2)
    por_prestador["media_uso"] = (por_prestador["quantidade_uso"] / por_prestador["qtd_procedimentos"]).round(2)
    media_esp = por_prestador.groupby("ESPECIALIDADE")[["custo_medio", "media_uso"]].mean()
    media_esp.columns = ["custo_medio_esp", "media_uso_esp"]
    por_prestador = por_prestador.merge(media_esp, on="ESPECIALIDADE", how="left")
    por_prestador["desvio_custo_pct"] = (
        (por_prestador["custo_medio"] - por_prestador["custo_medio_esp"]) / por_prestador["custo_medio_esp"] * 100
    ).round(1)
    ...
    resultado = por_prestador.sort_values("desvio_custo_pct", ascending=False)
    cols = ["CD_PRESTADOR", "UF", "CIDADE", "CLUSTER", "ESPECIALIDADE",
            "qtd_procedimentos", "valor_pago", "custo_medio", "custo_medio_esp",
            "desvio_custo_pct", "media_uso", "media_uso_esp", "desvio_uso_pct"]
    cols = [c for c in cols if c in resultado.columns]
    return resultado[cols].reset_index(drop=True)

def comparacao_mensal(df, coluna, volume_minimo=30):
    """Compara o último mês vs o anterior por coluna, respeitando volume mínimo."""
